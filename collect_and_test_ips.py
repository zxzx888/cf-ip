...
def calc_score(success_rate, tcp_lat, http_lat, speed):
    ...
    http_score = max(0, 20 - http_lat / 15)
...
def collect_ips():
    ...
    for url in URLS:
        try:
            ...
            for ip in valid_ips:
                ip_set.add(ip)
            print(f"✅ 原始提取: {len(raw_ips)} | 有效IP: {len(valid_ips)} | 累计去重: {len(ip_set)}")
...
def main():
    ...
    # ====================== 输出文件（核心修改：主文件无「分」相关内容） ======================
    # 1. 主文件：CloudflareSpeedTest.csv（完全移除「分」，仅保留IP、速度、延迟）
    with open("CloudflareSpeedTest.csv", "w", encoding="utf-8") as f:
        for item in top_results:
            # 格式：IP#速度Mbps·延迟ms，无任何得分/分相关内容
            f.write(f"{item['ip']}#{item['speed']}Mbps·{item['tcp_latency']}ms\n")

    # 2. 完整报告：ip_test_report.csv（保留所有得分、详细数据，完全不变）
    ...
    print("📁 ip_test_report.csv → 完整测试报告（保留全部得分、详细数据）")
